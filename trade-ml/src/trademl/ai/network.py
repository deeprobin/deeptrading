            ...
            if len(memory) == memory_size:
                if total_step % train_freq == 0:
                    ...
                    for i in memory_idx[::batch_size]:
                        ...
                        b_reward = np.array(
                            batch[:, 2].tolist(), dtype=np.int32)
                        b_obs = np.array(batch[:, 3].tolist(), dtype=np.float32).reshape(
                            batch_size, -1)
                ...
                if total_step % train_freq == 0:
                    ...
                    for i in memory_idx[::batch_size]:
                        ...
                        for j in range(batch_size):
                            ...
                            === """
                            target[j, b_pact[j]] = b_reward[j]+gamma * \
                                maxqs[j, indices[j]]*(not b_done[j])
                            """ >>> """
                        Q.reset()
                        loss = F.mean_squared_error(q, target)
                        total_loss += loss.data
